<?php
	...
	function __apath_expression($expression, $index, &$array) {
...
var_dump($expression);			
	 ...
	 * @return type
	 */
	function __apath_match(&$array, $parts, $partIndex, $isRooted, $pathParent = '') {
		...
		foreach ($array as $key => $value) {
			...
			if ($match && $expression) {
				$match = __apath_expression($expression, $index, $array);
			}
			...
			if ($match) {
				if ($partIndex >= count($parts) - 1) {
					...
					$matches += __apath_match($value, $parts, $partIndex + 1, $isRooted, $pathCurrent);
				}
			}

			if (!$isRooted && is_array($value)) {
				$matches += __apath_match($value, $parts, 0, $isRooted, $pathCurrent);
			}
	 ...
	 */
	function apath($array, $path, $pathkeys = true) {
		...
		$result = __apath_match($array, $path_parts, 0, $isRooted);
